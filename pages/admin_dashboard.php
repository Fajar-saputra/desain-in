...
// 1. OPTIMASI QUERY: Ambil semua hitungan statistik dalam 1 baris query
$statsData = $conn->query("
    SELECT 
        ...
        (SELECT COUNT(*) FROM orders WHERE order_status = 'selesai') as done_orders,
        (SELECT COALESCE(SUM(total_amount), 0) FROM orders WHERE payment_status = 'paid') as total_income
")->fetch(PDO::FETCH_ASSOC);

$counts = [
    'users'       => (int)$statsData['total_users'],
    'designers'   => (int)$statsData['total_designers'],
    'services'    => (int)$statsData['total_services'],
    'orders'      => (int)$statsData['total_orders'],
    'paid_orders' => (int)$statsData['paid_orders'],
    'done_orders' => (int)$statsData['done_orders'],
];

$income = (float)$statsData['total_income'];

// 2. QUERY PESANAN TERBARU
$recentOrders = $conn->query(
    'SELECT o.*, u.full_name AS customer_name, s.name AS service_name, c.category_name
     FROM orders o
...
// 3. FUNGSI HELPER (Label & Progress)
function admin_status_label($status) {
    $labels = [
        'baru' => 'Baru', 'diproses' => 'Diproses', 'revisi' => 'Revisi',
...
function order_progress($status) {
    $progress = [
        'baru' => 20, 'diproses' => 65, 'revisi' => 80, 
        'selesai' => 100, 'dibatalkan' => 0,
    ];
    return $progress[$status] ?? 10;
}

// 4. MAPPING DATA UNTUK STAT CARD
$stats = [
    [
        'title' => 'Pelanggan',
    ...
    <div class="admin-shell">
        ...
        <aside class="admin-sidebar">
            ...
            <nav class="admin-menu" aria-label="Admin navigation">
                ...
                <a class="admin-menu-link" href="#recent-orders">
                    <span class="admin-menu-icon">O</span> Pesanan
                </a>
                <a class="admin-menu-link" href="#recent-orders">
                    <span class="admin-menu-icon">G</span> Designer
                </a>
                <p class="admin-menu-label">Akun</p>
                <a class="admin-menu-link" href="/desainIn/pages/profile.php">
                    <span class="admin-menu-icon">A</span> Profil
                </a>
                <a class="admin-menu-link" href="/desainIn/logout.php">
                    <span class="admin-menu-icon">L</span> Logout
                </a>
            </nav>
        </aside>
        ...
        <main class="admin-main">
            <header class="admin-topbar">
                <!-- Tombol Toggle Sidebar dengan ID untuk JS -->
                <button id="toggleSidebar" class="admin-menu-button btn btn-light me-2" type="button">
